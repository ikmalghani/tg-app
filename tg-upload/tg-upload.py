...
Pyrogram: {get_version('pyrogram')} \
Prettytable: {get_version('prettytable')} \
Pillow: {get_version('pillow')} \
httpx: {get_version('httpx')} \
TgCrypto: {get_version('tgcrypto')} \
moviepy {get_version('moviepy')} \
"
...
with client:
  if args.login_only:
    # exit(str) prints to stderr and returns code 1 — treat success as exit 0
